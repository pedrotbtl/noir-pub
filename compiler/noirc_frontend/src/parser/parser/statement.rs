...
use crate::{
    ast::{
        AssignOp, AssignOpKind, AssignOpStatement, AssignStatement, BlockExpression,
        CallExpression, Expression, ExpressionKind, ForBounds, ForLoopStatement, ForRange, Ident,
        LValue, LetStatement, LoopStatement, Path, PathKind, PathSegment, Statement, StatementKind,
...
impl Parser<'_> {
    ...
    fn parse_verification_statement(
        comment: &str,
        func_name: String,
        loc: Location,
    ) -> Option<Statement> {
        ...
        if let Some((_, expression_str)) = comment.split_once(" ") {
            ...
            let expression_opt = expr_parser.parse_expression();
        ...
        let func = Box::new(Expression::new(
            ExpressionKind::Variable(Path {
                segments: vec![PathSegment {
                    generics: None,
                    ident: Ident::new(func_name, location),
                    location,
                }],
                kind: PathKind::Plain,
                location,
